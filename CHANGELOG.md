...
### ✨ 新增
- **可配置搜索引擎**: 支持通过配置文件自定义搜索引擎（顺序、URL、建议 API、图标）。
- **Chrome 商店合规**: 搜索功能重构，集成 Chrome Search API 以符合单一用途政策。

### 🔧 优化
- **搜索栏布局**: 恢复了大屏下的最大宽度和高度 (Refining Search Bar Layout)。
- **构建优化**: 优化了构建产物的 Chunk 大小 (Optimize Build Chunk Size)。

### 🐛 修复
- **搜索持久化**: 修复了搜索栏配置（如删除默认引擎）无法持久化的问题。
- **AI 助手跨域**: 修复了 AI 助手无法连接 API 的跨域 (CORS) 问题。
- **搜索建议跨域**: 修复了自定义搜索引擎建议的 CORS 问题。
- **图标交互**: 修复了图标交互区域过大导致的误触问题。
