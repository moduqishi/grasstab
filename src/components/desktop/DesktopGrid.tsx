...
const variants = {
    enter: (custom: { dir: number; width: number; screenW: number }) => {
        const offset = (custom.screenW + custom.width) / 2;
        return {
            x: custom.dir > 0 ? offset : -offset,
            opacity: 1,
            zIndex: 1,
        };
    },
    center: {
        ...
        opacity: 1,
        transition: {
            x: { type: "spring" as const, stiffness: 120, damping: 20, mass: 1 },
            opacity: { duration: 0.2 }
        }
    },
    exit: (custom: { dir: number; width: number; screenW: number }) => {
        const offset = (custom.screenW + custom.width) / 2;
        return {
            zIndex: 0,
            x: custom.dir < 0 ? offset : -offset,
        ...
        >
            {!isMobile && page > 0 && (
                <div 
                    ...
                    className="absolute top-1/2 -translate-y-1/2 p-2 sm:p-4 text-white/30 hover:text-white hover:bg-white/5 rounded-full cursor-pointer z-50 transition-all active:scale-95"
                    style={{ left: `calc(50% - ${gridWidth / 2}px - 80px)` }}
                >
            ...
            {!isMobile && page < totalPages - 1 && (
                <div 
                    onClick={(e) => { e.stopPropagation(); changePage(1) }} 
                    className="absolute top-1/2 -translate-y-1/2 p-2 sm:p-4 text-white/30 hover:text-white hover:bg-white/5 rounded-full cursor-pointer z-50 transition-all active:scale-95"
                    style={{ left: `calc(50% + ${gridWidth / 2}px + 20px)` }}
                >
            ...
            <div className="relative w-full h-full overflow-visible">
                <AnimatePresence initial={false} custom={{ dir, width: gridWidth, screenW: window.innerWidth }}>
                    <motion.div
                        key={page}
                        custom={{ dir, width: gridWidth, screenW: window.innerWidth }}
                        variants={variants}
                        initial="enter"
                        animate="center"
                        exit="exit"
                        className="absolute w-full h-full top-0 left-0 will-change-transform"
                        style={{ width: gridWidth + 'px', left: '50%', marginLeft: -(gridWidth / 2) }} // Center the grid
                    >
