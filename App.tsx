...
import { ChevronLeft, ChevronRight, X, Minus, Check, Edit3, Plus } from 'lucide-react';
import { DEFAULT_WALLPAPER, SEARCH_ENGINES, DEFAULT_SHORTCUTS, DEFAULT_DOCK } from './constants';
    ...
    return (
        ...
        >
            ...
            {sysSettings.showSearchBar && (
                <div className="absolute top-0 w-full pt-[8vh] flex flex-col items-center z-10 transition-opacity duration-300" style={{ opacity: isAnyWindowMaximized ? 0 : 1, pointerEvents: isAnyWindowMaximized ? 'none' : 'auto' }}>
                    ...
                    <div className="w-[90%] max-w-xl bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl flex items-center p-1.5 shadow-2xl transition-all duration-300 focus-within:bg-white/20 focus-within:scale-105" onClick={e=>e.stopPropagation()}>
                        <button onClick={()=>setEngine(prev => {
                            ...
                            return keys[nextIdx];
                        })} className="w-10 h-10 flex items-center justify-center hover:bg-white/10 rounded-xl transition-colors text-white font-bold"><span className="text-xl pb-1">{SEARCH_ENGINES[engine].icon}</span></button>
                        <input className="flex-1 bg-transparent border-none outline-none text-white px-3 text-lg placeholder-white/40 font-light h-10" placeholder={`Search ${SEARCH_ENGINES[engine].name}...`} value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>e.key==='Enter' && window.location.assign(SEARCH_ENGINES[engine].url+encodeURIComponent(search))} />
            ...
            {/* Middle Zone - Grid */}
            <div 
                className={`absolute top-[380px] w-full max-w-[95%] xl:max-w-[1400px] left-1/2 -translate-x-1/2 z-10 ${sysSettings.showDock ? 'bottom-[180px]' : 'bottom-[40px]'}`}
                onWheel={(e) => {
            ...
            {/* iPadOS Style Dock */}
            <div className={`absolute bottom-8 left-1/2 -translate-x-1/2 z-[9999] transition-transform duration-500 cubic-bezier(0.32, 0.72, 0, 1) ${!sysSettings.showDock || isAnyWindowMaximized ? 'translate-y-[250%]' : 'translate-y-0'}`}>
                <div 
